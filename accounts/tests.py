...
class SnippetAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        self.snippet = Snippet.objects.create(
            title="Alice's snippet", code='print("hello alice")', owner=self.user1
        )
    ...
    def test_unauthenticated_cannot_create_snippet(self):
        ...
        response = self.client.post(
            self.list_url, {"title": "Hack", "code": 'print("evil")'}, format="json"
        )
        self.assertEqual(
            response.status_code, status.HTTP_403_FORBIDDEN
        )  # or 401 if you change settings

    def test_unauthenticated_cannot_update_snippet(self):
        """PUT /api/snippets/<pk>/ should require authentication"""
        response = self.client.put(
            self.detail_url,
            {"title": "Hacked title", "code": self.snippet.code},
            format="json",
        )
    ...
    def test_authenticated_can_create_snippet(self):
        ...
        response = self.client.post(
            self.list_url, {"title": "New snippet", "code": "x = 42"}, format="json"
        )
    ...
    def test_owner_can_update_snippet(self):
        ...
        response = self.client.put(
            self.detail_url,
            {"title": "Updated title", "code": self.snippet.code},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_non_owner_cannot_update_snippet(self):
        ...
        response = self.client.put(
            self.detail_url,
            {"title": "Bob wants to hack", "code": self.snippet.code},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_login_required_for_write_actions(self):
        ...
        response = self.client.post(
            self.list_url, {"title": "No auth", "code": "fail"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        ...
        self.client.login(username="alice", password="alicepass")
        response = self.client.post(
            self.list_url, {"title": "With auth", "code": "win"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
