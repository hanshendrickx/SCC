...
class ProjectTreeGenerator:
    ...
    def generate_tree(
        self, start_path: Path, output_file: str = None, max_depth: int = 3
    ) -> str:
    ...
    def should_show_directory(self, path: Path) -> bool:
        return path.name in self.always_show_dirs or (
            path.name not in self.exclude_dirs
            and not any(excluded in path.parts for excluded in self.exclude_dirs)
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate JPNB-DYAD project tree structure"
    )
    parser.add_argument(
        "--path",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="HELPFILES/project_structure.txt",
    ...
    tree = generator.generate_tree(Path(args.path), args.output, args.max_depth)
